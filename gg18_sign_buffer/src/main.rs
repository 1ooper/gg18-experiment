...
use reqwest::Client;
use std::{env, fs, time, collections::HashMap};
use std::sync::RwLock;
...
async fn main() {
    ...
    {   
        ...
        for i in 0..sign_number {
            ...
            tx_buffer.insert(uuid, message_str.clone());
        }
    }
    rocket::build()
        .mount("/", routes![act_tx])
...
fn act_tx(db_mtx: &State<RwLock<HashMap<String, String>>>, tx: Json<String>) -> Json<Result<String, ()>>{
    ...
    let message = tx_buffer.get(&act_tx).unwrap();
    distributed_sign(&act_tx, &message, "keys1.store");
